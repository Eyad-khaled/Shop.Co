...
const ProductDetails = ({ product }: ProductDetailsProps) => {
  ...
  return (
    <div className="w-[90%] mx-auto">
      <div
        className={`absolute bottom-6 left-4 bg-green-500 text-white py-2 px-4 rounded-[30px] flex justify-between items-center gap-4  transition-all transition ease-in-out duration-[1s] ${IsSuccesful ? "opacity-1" : "opacity-0"}`}
      >
      ...
      <div className="flex justify-center  lg:gap-4 mt-14 lg:flex-row flex-col gap-8">
        <div className="images flex flex-col-reverse lg:flex-row justify-center items-center gap-4">
          {product.images.length > 1 && (
            <div className="three-imgs flex justify-center items-center lg:flex-col flex-row gap-2">
              {product.images.map((image: string, index: number) => (
                <div
                  key={index}
                  className="bg-[#F0EEED] rounded-lg cursor-pointer"
          ...
          <div className="current-img bg-[#F0EEED] px-8 rounded-lg flex justify-center items-center">
        ...
        <div className="details flex flex-col justify-between basis-[60%]">
          ...
          <div className="price flex items-center gap-8">
            <h1 className="current-price text-[20px] font-[700]">
              $
              {(
                product.price *
            ...
            <h1 className="current-price text-[20px] font-[700] opacity-40 line-through">
              ${product.price.toFixed(2)}
            </h1>
            <h1 className="current-price text-[16px] font-[500] text-[#FF3333] bg-lightRed px-2 rounded-[30px]">
              - {Math.ceil(product.discountPercentage)}%
            </h1>
          </div>
          <p className="py-4 text-lg font-semibold">
            {" "}
            <span className="opacity-60">Brand</span> : {product.brand}
          </p>
          ...
          <div className="add-to-cart flex pt-16 items-center gap-4">
            ...
            <div
              ...
              className="add-to-cart cursor-pointer flex-grow text-center bg-black text-white rounded-[50px] py-4"
            >
